	...
	function AudioApiElement(audioElement) {
   	...
   	scrollPosition = currentTrackEl.position().top;*/
    	...
    	removeAttr('data-current-track');
			...
			if (window.innerHeight >= 720 && window.innerWidth > window.innerHeight || window.innerWidth >= 1000 && window.innerHeight > window.innerWidth) {
				drawEq1();
				drawEq2();
			}
			drawEq3();
 ...
 */
	...
	if (localStorage.getItem('playerState') == undefined) {
		...
		$('.playlistsPanel').find('.list').
		// mCustomScrollbar('scrollTo', __playlists[playerState.currentPlaylist].scrollPosition);
		mCustomScrollbar('scrollTo', $playlistsPanel.find('[data-current]').attr('data-scroll-left'));
